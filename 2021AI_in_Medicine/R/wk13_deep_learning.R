...
seedNum <- 777
torch_manual_seed(seedNum)
set.seed(seedNum)


# use your own kaggle.json here
pins::board_register_kaggle(token = "./extras/kaggle.json")
...
device <- torch_device(if(cuda_is_available()) "cuda" else "cpu") #'gpu' if 'cuda' generates error
